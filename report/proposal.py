...
def _s_원인(topic, ev, cards, human):
    ...
    # ★ 두 번째 판정도 같은 자리를 짚었다 — «상관인지 인과인지 보완 설명이
    #   필요하다». 문장은 이미 있었는데 **그 낱말이 없었다.** 읽는 사람은
    #   자기가 찾는 낱말로 찾는다. 뜻이 같아도 낱말이 다르면 없는 것이다.
    # ★ 세 번째 판정도 같은 자리를 짚었다 — «다른 설명일 가능성을 견줘 본
    #   결과가 있어야 한다». **그 결과는 이미 있었다.** 규모 절 가정 표와
    #   위험 절에 있었고, 「대안 설명」 이라는 낱말이 없었을 뿐이다.
    #   B 는 두 종류다 — 자리가 뒤인 것과 낱말이 다른 것. 여기는 둘 다였다.
    꼬리 = ["칸을 나눠 견준 값이라 상관이지 인과가 아닙니다 "
            "(한쪽을 바꿔 보고 잰 값이 아닙니다)",
            "대신 설명할 만한 것을 눌러 본 결과는 아래 「흔들어 봤더니」 에 "
            "있습니다",
            # ★ 또 같은 자리를 짚었다 — «제안이 상관에 기대고 있다». 그 답은
            #   이미 위험 절에 한 문단으로 있다: 이 제안은 «낮추면 통과한다»
            #   가 아니라 «낮은 쪽에 더 낸다» 라서 인과를 안 쓴다.
            #   ★ 처음에는 그 문단을 **가리키기만** 했다. 안 먹혔다 —
            #     가리키는 문장은 «저기 있습니다» 지 논거가 아니다. 읽는
            #     사람은 이 절에서 판정하고, 이 절에 논거가 없으면 없는 것이다.
            #     그래서 **논거 자체를 여기서 한 번 말하고** 자세한 것을
            #     가리킨다. 옮기는 것이 아니라 한 줄로 세우는 것이다.
            f"이 제안이 기대는 것은 「{c['축']}"
            f"{_josa(c['축'], ('을', '를'))} 바꾸면 달라진다」가 아니라 "
            f"「지금 나은 쪽으로 옮긴다」 라서, 이 관계가 앞으로도 "
            f"이어지기만 하면 섭니다 — 인과를 몰라도 되는 까닭과 "
            f"이어지지 않을 때 접는 법은 「이 판단이 틀릴 수 있는 자리」 에 "
            f"적었습니다"]
    if hidden:
