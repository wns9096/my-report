...
STEPS = [
    ...
    # Day4 — 이 문서는 밖으로 나간다. «잘 만들어졌는가» 와 «내보내도 되는가»는
    # 다른 질문이라 검사를 따로 둔다.
    ("9주차 Day4 — 제출 전 확인 (빈칸·수업 용어·결정 동사·식별 정보)",
     "checks/w9d4_submit.py"),
    ("9주차 Day4 — 받은 지적이 문서에서 실제로 해소됐는가",
     "checks/w9d4_answers.py"),
    # Day5 — 9주가 끝났는가. 산출물이 **있는가**를 본다. 잘 만들어졌는지는
    # 위의 열둘이 보고, 이것은 «빠진 것이 없는가»만 본다. 둘은 다른 질문이다.
    ("9주차 Day5 — 9주 마감 (산출물 다섯 · 여덟 장 · 적용 다섯 줄)",
     "checks/w9d5_close.py"),
    # 교안 14장 — 위의 열셋은 «이 앱이 내 규칙을 지키는가»를 본다. 이것은
    # «교재가 시킨 것을 다 했는가»를 본다. 겹치는 항목은 새로 세지 않고
    # 위의 검사에 맡긴다 — 두 곳에서 세면 언젠가 두 값이 갈린다.
    ("교안 14장 — 완주 체크리스트 (재현 · 심은 것 · 되돌림 · 배포)",
     "checks/course_final.py"),
]
