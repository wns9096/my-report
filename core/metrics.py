...
def biggest_gap(df):
    ...
    원값으로 뺀 10.4%p 를 격차라고 적으면, 읽는 사람이 암산해서 맞춰 볼 수
...
def _u(topic):
    """«통과 1건» 인지 «통과 1명» 인지. 세는 단위를 문장마다 다시 안 정한다."""
    return f"1{topic.get('단위', '건')}"


def _rng(t):
    """(작은 값, 큰 값) 을 «a~b» 로. 두 값이 같으면 하나만 적는다."""
    a, b = t
    fmt = lambda v: f"{v:g}"
    return fmt(a) if a == b else f"{fmt(a)}~{fmt(b)}"


def hours_cost(topic):
    """비용을 **시간**으로 환산한다. 금액은 못 낸다 — 시급이 데이터에 없다.

    ★ 건당 시간은 추정이고 범위다(config.HOURS_PER_APPLICATION). 그래서
      결과도 범위로 낸다. 범위로 내면 읽는 사람이 «얼마다» 가 아니라
      «얼마에서 얼마다» 로 읽는다. 그게 이 값의 실제 모양이다.

    ★ 그리고 **범위와 무관한 값 하나**를 따로 낸다. 통과 1건에 드는 시간이
      몇 % 줄어드는지는 건당 시간이 약분돼 사라진다 — 2시간이든 4시간이든
      같은 수가 나온다. 추정이 틀려도 안 바뀌는 것이 하나 있다는 뜻이고,
      그것이 이 제안을 «추정에 기대지 않는 만큼» 이다.

    돌려주는 것은 dict 또는 None. 셀 것이 없으면 None 이고, 그때 문서는
    이 줄을 아예 안 만든다 — 빈칸으로 두지 않는다.
    """
    도달, 분모, 격차 = topic.get("도달"), topic.get("분모"), topic.get("격차")
    if not 분모 or 도달 is None or 격차 is None:
        return None
    지금 = _pp(도달 / 분모)                 # 화면에 찍히는 자리까지 맞춘다
    바꾼뒤 = round(지금 + _pp(격차), 1)     # 격차도 같은 자리에서 왔다
    if 지금 <= 0 or 바꾼뒤 <= 0:
        return None
    lo, hi = config.HOURS_PER_APPLICATION

    def 통과1건(h, 율):
        return round(h / (율 / 100), 1)

    지금들 = (통과1건(lo, 지금), 통과1건(hi, 지금))
    뒤들 = (통과1건(lo, 바꾼뒤), 통과1건(hi, 바꾼뒤))
    return {
        "건당": (lo, hi),
        "지금율": 지금, "바꾼뒤율": 바꾼뒤,
        "지금": 지금들, "바꾼뒤": 뒤들,
        "아끼는": (round(지금들[0] - 뒤들[0], 1), round(지금들[1] - 뒤들[1], 1)),
        # 건당 시간이 약분된다. 추정이 틀려도 이 수는 안 바뀐다.
        "줄어드는비율": round((1 - 지금 / 바꾼뒤) * 100, 1),
        "분모시간": (round(분모 * lo), round(분모 * hi)),
        "분모": 분모, "단위": topic.get("단위", "건"),
    }
...
def topic_evidence(tables, topic):
    ...
    if topic.get("규모_연간건수") is None:
        ...
        # ★ 가정과 흔들기는 **한 줄에 묶는다.** 예전에는 두 목록을 나란히 두고
        #   「같은 순서로 맞춘다」고 주석을 달아 뒀는데, 흔들기 가운데에 한 줄을
        #   끼우고 가정은 끝에 붙이자 한 칸씩 밀렸다 — 금액 가정에 엉뚱한
        #   결과가 붙어 있었다. 주석은 코드를 안 막는다. 묶어 두면 밀 자리가
        #   없다.
        #   짝이 없는 가정은 「안 해 봤습니다」 다. 빈칸으로 두면 안 한 것인지
        #   못 한 것인지 읽는 사람이 알 수 없다.
        안해봤다 = "안 해 봤습니다."
        짝 = [
            ...
            # ★ 예전에는 이 줄이 «금액으로 환산하지 않았습니다» 하나였다.
            #   맞는 말이지만 읽는 사람이 묻는 것은 «그럼 얼마가 드는가» 다.
            #   금액은 여전히 못 낸다 — 시급이 없다. **시간은 낼 수 있다.**
            #   그리고 범위로 받은 값은 **양 끝을 다 넣어 본다.** 그게 범위로
            #   받은 이유다. 한쪽만 넣으면 범위로 받은 뜻이 없어진다.
            ((f"금액으로는 환산하지 않았습니다 — 시급을 적어 둔 항목이 "
              f"없습니다. 대신 **시간으로** 환산했습니다: 통과 {_u(topic)}을 "
              f"얻는 데 지금 {_rng(hc['지금'])}시간, 바꾼 뒤 "
              f"{_rng(hc['바꾼뒤'])}시간입니다. 지원 1건에 드는 "
              f"{_rng(hc['건당'])}시간은 데이터가 아니라 지원하는 본인이 말한 "
              f"**추정**이고, 다음 달 지원부터 건별로 적어 한 달 뒤 셉니다.",
              f"건당 {hc['건당'][0]:g}시간·{hc['건당'][1]:g}시간 양 끝을 다 "
              f"넣어 봤습니다. 통과 {_u(topic)}에 드는 시간이 "
              f"{hc['지금'][0]}→{hc['바꾼뒤'][0]}시간 · "
              f"{hc['지금'][1]}→{hc['바꾼뒤'][1]}시간으로 **어느 끝에서도 "
              f"줄어드는 쪽**입니다. 줄어드는 비율 {hc['줄어드는비율']}% 는 "
              f"건당 시간이 약분돼 **추정이 틀려도 안 바뀝니다**.")
             if hc else
             ("금액으로 환산하지 않았습니다. 건당 금액을 적어 둔 항목이 "
              "없습니다 (단가 미확보).",
