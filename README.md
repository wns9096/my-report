...
| `day2_crosscheck.py` | 앱의 숫자가 7주차 손계산과 같은가 (7항목) |
...
| `app_audit.py` | 나흘치 규칙 10개를 앱이 실제로 지키는가 (`/앱점검` 의 기계 점검부) |
