...
| `app_audit.py` | 나흘치 규칙 14개를 앱이 실제로 지키는가 (`/앱점검` 의 기계 점검부) |
...
├── 발견.md                  ★ 9주차 — 근거 넷을 채운 발견 5개 · 못 쓰는 숫자 8개
├── 판단기준.md              ★ 도메인 단어 없는 판단 기준 여덟 + 9주차 것
├── ★_채운자리.md            도메인을 바꿀 때 고칠 곳, Day별 정리
└── 발표.md                  토요일 발표 다섯 질문에 대한 답
```

`/앱점검` 슬래시 커맨드는 `~/.claude/commands/앱점검.md` 에 있다.
기계 점검 14개를 돌린 뒤, 코드가 판정할 수 없는 다섯 가지를 사람이 보게 한다.
